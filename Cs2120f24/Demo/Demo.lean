...
/-
Now let's build some new propositions and construct proofs of them
-/
...
truth (is constructible from a proof) of the ∨.
This is the *or elimination* inference rule.
-/
example : (ItsRaining ∨ SprinkerIsRunning → StreetsAreWet) :=
  λ hor =>          -- assume premise (with hor as proof of it)
    Or.elim hor     -- prove by or intro (case analysis)
      (sorry)           -- conclusion holds in first case
      (sorry)           -- conclusion holds in second case
...
This is one of many examples of the utility of a higher
order logic for teaching. You just can't write this kind
of thing in FOTFPL.
-/
...
/-
One of the most fundamental "moves" in any kind of
mathematical reasoning is to *apply* a theorem to a
special case to obtain a proof *for that case*. This
is the operation of universal instantiation.
...
-- Apply theorem to proposition-valued arguments
#check and_commutes ItsRaining SprinklerRunning

/-
Hover your cursor over #check to see that Lean now
accepts the following judgment:

and_commutes ItsRaining SprinklerRunning :
  ItsRaining ∧ SprinklerRunning ↔
  SprinklerRunning ∧ ItsRaining

The function application term, *(and_commutes ItsRaining
SprinklerRunning)* is a proof of the special case. Again
the higher-order nature of the logic is evident here, in
the form of a function that takes propositions (which are
types) as its arguments.
-/

/-
A look ahead: the induction inference rules for the
inductively defined data types, Nat and List α (the
polymorphic type of lists with elements of type α).
...
theorem and_left : ∀ (P Q : Prop), P ∧ Q → P := fun P Q hpq => hpq.left
theorem and_right : ∀ (P Q : Prop), P ∧ Q → Q := fun _ _ hpq => hpq.right
