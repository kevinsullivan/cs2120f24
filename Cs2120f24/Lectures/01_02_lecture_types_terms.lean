/-!
# Types, The Terms Thereof, And Their Interpretations


We distinguish between different *types* of things all the
time. For example, we might use the term *dog* to refer to
the type of all and only tbose things that we want to judge
to be honest-to-goodeness dogs.
-/
...
/-!
This course in discrete mathematics is not just for computer
science students, in particular (because we teach it taught in
a way that is meant both to appeal to and to exploit students'
intution for computing with data values.

  explicitly and rigorous establishes in your understanding
the notions of propositions as types, proofs as *computational*
objects of such types, static type checking of purported proofs
against the corresponding propositions (by type checking)
the beautiful core concepts of *computation* --
