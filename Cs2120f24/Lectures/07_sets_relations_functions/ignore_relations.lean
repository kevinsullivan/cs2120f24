...
When we say that, or ask whether, a relation sastisfies
a given predicate, we are asking if it has the property
that the specification addresses.

The property, of a binary relation, of being empty is
specified by the predicate that always reduces to False
and is therefore never satisfiable. There are no proofs
of any membership propositions of this kind.

Note that here we use the syntactic flexibility of Lean
to infer and name the type arguments that make the isEmpty
relation applicable to pairs of values of any two types
at all.

While we can specify any relation as a special-purpose,
one-off predicate, it's especially powerful to be able
to generalize across all possible types of pairs. Doing
this establishes a broadly general concept applicable to
the entire universe of types of binary relations.
-/

def isEmpty {α β : Type} : Rel α β := fun a b => False
